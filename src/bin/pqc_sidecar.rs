...
use pqcrypto_dilithium::dilithium2::{
    detached_sign, keypair as dilithium2_keypair, verify_detached_signature, DetachedSignature, PublicKey as DilithiumPublicKey, SecretKey as DilithiumSecretKey
};
use pqcrypto_kyber::kyber512::{
    decapsulate, encapsulate, keypair as kyber512_keypair
};
...
static KEYPAIR: Lazy<(DilithiumPublicKey, DilithiumSecretKey)> = Lazy::new(|| {
    dilithium2_keypair()
});
...
async fn verify(Json(payload): Json<VerifyRequest>) -> Json<VerifyResponse> {
    let mut valid = false;
    if let (Ok(sig_bytes), Ok(pk_bytes)) = (STANDARD.decode(&payload.signature_b64), STANDARD.decode(&payload.public_key_b64)) {
        if let (Ok(sig), Ok(pk)) = (DetachedSignature::from_bytes(&sig_bytes), DilithiumPublicKey::from_bytes(&pk_bytes)) {
